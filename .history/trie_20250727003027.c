...
void spellCheck(TrieNode* root, const char* word) {
    ...
    for (int i = 0; i <= strlen(word); i++) {
        ...
        for (char c = 'a'; c <= 'z'; c++) {
            ...
            memove(suggestion + i + 1, suggestion + i, strlen(suggestion) - i + 1);
            suggestion[i] = c;
    ...
    for (int i = 0; i <= strlen(word); i++) {
        strcpy(suggestion, word);
        memove(suggestion + i, suggestion + i + 1, strlen(suggestion) - i);
        if (search(root, suggestion)) {
            printf("%s\n", suggestion);
        }
    }
...
int main() {
    ...
    // Spell checker demo
    printf("==== Spell Checker Demo =====\n");
    spellCheck(root, "kat");

    // Word prediction demo
    printf("==== Word Prediction Demo =====\n");
